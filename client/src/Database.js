...
class Database {
  constructor() {
    ...
    state.currentEvent =
      JSON.parse(window.localStorage.getItem("event")) || emptyEvent;
    ...
    this.apiCall(APICALLS.CLIENT_STATUS, {})
      .then(data => {
  ...
  client_getUsers() {
    return new Promise((resolve, reject) => {
      this.apiCall(APICALLS.USER_LIST, {})
        .then(result => {
          var users = [];
  ...
  user_getEvents() {
    return new Promise((resolve, reject) => {
      this.apiCall(APICALLS.EVENT_LIST, {})
        .then(result => {
  ...
  push_apiCall = () => {
    ...
    for (var v = 0; v < actionsToDo.length; v += 1) {
      ...
      this.apiCall(actionsToDo[v].action, actionsToDo[v].params)
        .then(data => {
          console.log("SUCCESS");
        })
        .catch(data => {
