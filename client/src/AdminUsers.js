...
class AdminUsers extends Component {
  ...
  createUser = () => {
    ...
    Alert.success(
      "Creating user " + this.state.username.trim(),
      ALERT_SETTINGS
    );
    var params = {
    ...
    Database.client_createUser(params)
      .then(() => {
        Alert.success(
          "Created user " + this.state.username.trim(),
          ALERT_SETTINGS
        );
        this.getUsers();
      })
      .catch(() => {
        Alert.error(
          "Couldn't create user " + this.state.username.trim(),
          ALERT_SETTINGS
        );
      });
  };
  ...
  applyFunction = () => {
    ...
    var userJSON;
    if (
      this.state.user_action === "delete" &&
      window.confirm("Are you sure you want to delete a user?")
    ) {
      userJSON = { id: user.id };
      Database.client_deleteUser(userJSON)
        .then(() => {
          Alert.success(
            "Deleted user " + this.state.username.trim(),
            ALERT_SETTINGS
          );
          this.getUsers();
        })
        .catch(() => {
          Alert.error(
            "Couldn't delete user " + this.state.username.trim(),
            ALERT_SETTINGS
          );
        });
    } else if (this.state.user_action === "assign") {
      userJSON = { id: user.id, event_id: this.state.event_id, assign: true };
      Database.client_updateUserEventAssignment(userJSON)
        .then(() => {
          Alert.success(
            "Assigned user " + this.state.username.trim(),
            ALERT_SETTINGS
          );
          this.getUsers();
        })
        .catch(() => {
          Alert.error(
            "Couldn't assign user " + this.state.username.trim(),
            ALERT_SETTINGS
          );
        });
      ...
      Database.client_updateUserEventAssignment(userJSON)
        .then(() => {
          Alert.success(
            "Unssigned user " + this.state.username.trim(),
            ALERT_SETTINGS
          );
          this.getUsers();
        })
        .catch(() => {
          Alert.error(
            "Couldn't unassign user " + this.state.username.trim(),
            ALERT_SETTINGS
          );
        });
    }
  ...
  render() {
    ...
    const users_columns = [
      ...
      },
      {
        ...
        accessor: "events",
        Cell: row => {
          return row.value.map(event => (
            <span>
              <Badge color="secondary">
                {Database.event_idToName(event).name}
              </Badge>{" "}
            </span>
          ));
        }
      },
      {
        Header: "",
        Cell: props => (
          <Button onClick={e => this.selectUser(props.original)} color="danger">
            Select
          </Button>
    ...
    return (
      <Container>
          ...
          <InputGroup>
            ...
            <InputGroupAddon addonType="append">
              ...
              >
                ...
                <option key="2" value="assign">
                  Assign Event
                </option>
                <option key="3" value="unassign">
                  Unassign Event
                </option>
              </CustomInput>
            </InputGroupAddon>
            ...
            <Button
              disabled={this.state.currentUser === noUser}
              color="info"
        ...
        <Col>
          ...
          <ReactTable data={this.state.users} columns={users_columns} />
        </Col>
      </Container>
    );
  }
