...
class Dashboard extends Component {
  ...
  refreshPrinters = () => {
    this.setState({
      ...this.state,
      printers: [{ name: Constants.DONT_PRINT_NAME }].concat(
  ...
  updateApplyDisabled = () => {
    if (
      this.state.checkin !== "doing nothing" ||
      this.state.tags.length > 0 ||
      this.state.printerName !== Constants.DONT_PRINT_NAME
    ) {
      this.setState({
        applyDisabled: false
      });
  ...
  updateFilteredAttendees = searchValue => {
    ...
    this.setState({
      ...this.state,
      filteredAttendees: this.state.attendees.filter(attendee =>
  ...
  onSearchKeyPress = e => {
    if (e.key === "Enter") {
      if (
        this.state.filteredAttendees.length === 1 &&
  ...
  applyFunctionConfirm = attendee => {
    ...
    if (
      ...
      !attendee.updateCheckin(this.state.day, true, true)
    ) {
      Constants.AlertError("User has already checked in");
      error = "User has already checked in";
    }
    if (
      this.state.checkin === "uncheckin" &&
      !attendee.updateCheckin(this.state.day, false, true)
    ) {
      Constants.AlertError("User has not been checked in");
      error = "User has not been checked in";
    }
    if (tags.length > 0 && !attendee.updateTags(tags, true)) {
      Constants.AlertError("Tag already exists");
  ...
  applyFunctionActual = () => {
    ...
    if (tags.length > 0) {
      ...
      tags = sortedTags[0]
        ...
        .concat(
          sortedTags[1].map(tag => {
            return { tag: tag, key: "UNACTION" };
          })
        );
    ...
    if (Database.client_loggedIn()) {
      ...
      const columns = [
          ...
          Cell: row => {
            ...
            while (Math.pow(2, dayCount) <= value) {
              if ((Math.pow(2, dayCount) & value) > 0) {
                ret.push(
                  <span>
                    <Badge color="primary">{dayCount}</Badge>{" "}
                  </span>
                );
              }
              dayCount += 1;
        ...
        },
        {
          ...
          accessor: "tags",
          Cell: row => {
            return row.value.split(";").map(badge => (
              <span>
        ...
        },
        {
          ...
          Cell: props => (
            <Button
              onClick={e => this.applyFunctionConfirm(props.original)}
      ...
      let printerOptions = this.state.printers.map((printer, index) => (
        ...
        <Container>
          ...
          <Col>
            <InputGroup>
              ...
              <InputGroupAddon addonType="prepend">
                ...
                >
                  <option key="1" value="doing nothing">
                    No Action
                  </option>
                  <option key="2" value="checkin">
                    Checkin
                  </option>
                  <option key="3" value="uncheckin">
                    unCheckin
                  </option>
                </CustomInput>
              </InputGroupAddon>
              <Input
                placeholder="Tags separated by commas (i.e. swag, !swag remove tags)"
                name="tags"
                value={this.state.tags}
                onChange={this.handleChange}
              />
              <InputGroupAddon addonType="prepend">
                <InputGroupText>Printer:</InputGroupText>
                <Input
                  type="select"
                  name="printerName"
                  onChange={this.handleChange}
            ...
            <Input
              ...
              placeholder="Search for anything! ~1 will check for status for day 1"
            />
            <br />
            We are <b>{this.state.checkin}</b> for day <b>{this.state.day}</b>{" "}
            for <b>{Database.client_currentEvent().name}</b> with these tags:{" "}
            <b>[{this.state.tags}]</b> Printing to{" "}
            <b>{this.state.printerName}</b>
          </Col>
          <br />
          <Col>
            {/* Actual Results */}
            <h3>Results ({this.state.filteredAttendees.length})</h3>
            <ReactTable data={this.state.filteredAttendees} columns={columns} />
          </Col>
          <Popup
            open={this.state.view_confirmApply}
            modal
          ...
          >
            <Container>
              ...
              <p>
                {" "}
                Checkin: {this.state.checkin} for DAY {this.state.day}
              </p>
              ...
              <center>
                ...
                <Button color="primary" onClick={this.applyFunctionActual}>
                  Apply Function
                </Button>
              </center>
            </Container>
            ...
            <PopoverBody>
              <FormGroup>
                ...
                <InputGroup>
                  ...
                  Total actions waiting to be pushed{" "}
                  {Database.get_apiCallLength()}
                  <Button
                    onClick={() => {
                      Database.clear_apiCall();
                      Alert.success("Cleared queued requests", ALERT_SETTINGS);
                    }}
                  >
                    {" "}
                    Force Clear{" "}
