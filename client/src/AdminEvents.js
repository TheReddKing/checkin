...
class AdminEvents extends Component {
  ...
  createEvent = () => {
    ...
    Database.client_createEvent(params)
      .then(() => {
        Alert.success(
          "Created event " + this.state.name.trim(),
          ALERT_SETTINGS
        );
        this.getEvents();
      })
      .catch(() => {
        Alert.error(
          "Couldn't create event " + this.state.name.trim(),
          ALERT_SETTINGS
        );
      });
  };
  ...
  applyFunction = () => {
    ...
    var eventJSON;
    if (
      this.state.event_action === "delete" &&
      window.confirm("Are you sure you want to delete the event?")
    ) {
      eventJSON = { id: event.id };
      Database.client_deleteEvent(eventJSON)
        .then(() => {
          Alert.success(
            "Deleted event " + this.state.name.trim(),
            ALERT_SETTINGS
          );
          this.getEvents();
        })
        .catch(() => {
          Alert.error(
            "Couldn't delete event " + this.state.name.trim(),
            ALERT_SETTINGS
          );
        });
    }
  ...
  render() {
    const event_columns = [
      ...
      { Header: "Id", accessor: "id" },
      {
        ...
        Cell: props => (
          <Button
            onClick={e => this.selectEvent(props.original)}
            color="danger"
          >
            Select
          </Button>
    ...
    return (
      <Container>
        ...
        <FormGroup>
          <InputGroup>
            <InputGroupAddon addonType="prepend">
              <InputGroupText>
                {" "}
                Selected Event: {this.state.currentEvent.name}{" "}
              </InputGroupText>
              <CustomInput
                id="event_options"
                type="select"
                name="action"
                value={this.state.event_action}
                onChange={this.handleChange}
              >
            ...
            <Button
              disabled={this.state.currentEvent === noEvent}
              color="info"
        ...
        <Col>
          ...
          <ReactTable data={this.state.events} columns={event_columns} />
        </Col>
      </Container>
    );
  }
